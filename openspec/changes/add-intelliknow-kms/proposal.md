...
- **Single configuration file**: one `config.yaml` holds every non-secret tunable. Secrets live in `.env` or encrypted integration storage. Intent spaces, confidence threshold, and relevance floor apply without restart; provider, model, storage, chunking, and retrieval-topology changes require restart.
...
- **New external dependencies**: a Telegram bot token from BotFather, an AI provider API key, an admin password and credential-encryption key, and an Azure Bot registration plus reachable HTTPS endpoint for real Teams acceptance. Docker is not required.
