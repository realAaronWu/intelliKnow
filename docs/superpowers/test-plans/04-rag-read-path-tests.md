...
| # | Test | Expected |
|---|---|---|
| 4.1 | Best cosine above floor | passes |
| 4.2 | Best cosine below floor | fails |
| 4.3 | Exactly at floor | passes |
| 4.4 | **Gate ignores the fused score** | construct high fused rank with sub-floor cosine → **fails**. A gate reading the fused score would pass this |
| 4.5 | Keyword-only hits | do not satisfy the gate alone |
| 4.6 | Empty hits | fails |
...
| # | Test | Expected |
|---|---|---|
| 9.1 | Returns slug, confidence, reasoning | all three populated |
| 9.2 | **Prompt built from live config** | recorded prompt contains every space's name, description, and keywords |
| 9.3 | Keyword edit takes effect | after `ConfigService.update`, the next prompt contains the new keyword — no restart |
| 9.4 | Unknown slug | treated as below-threshold; anomaly recorded |
| 9.5 | Provider failure | `failed=True`, fallback slug, **no exception raised** |
| 9.6 | Timeout | same as 9.5 |
| 9.7 | Uses classify model | provider constructed with `model_classify` |
...
| # | Test | Expected |
|---|---|---|
| 11.1 | **Concurrency** | with classification and embedding each stubbed to ~200ms, total elapsed is materially below 400ms |
